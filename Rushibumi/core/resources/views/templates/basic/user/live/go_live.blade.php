            ...
            <div class="card-body">
                <form id="goLiveForm">
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="form-group mb-3">
                                <label class="form-label">@lang('Stream Title') <span class="text-danger">*</span></label>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        <label class="form-label">@lang('Visibility') <span class="text-danger">*</span></label>
                    ...
                    <div class="thumbnail-section">
                        <h4 class="thumbnail-heading">@lang('Thumbnail')</h4>
                        <p class="thumbnail-description">
                            @lang('Select or upload a picture that represents your stream. A good thumbnail stands out and draws viewers\' attention.')
                            <a href="#" class="thumbnail-learn-more">@lang('Learn more')</a>
                        </p>
                        <div class="thumbnail-upload-container">
                            <div class="thumbnail-upload-box" id="thumbnailUploadBox">
                                ...
                                <label for="thumbnailInput" class="thumbnail-upload-label">
                                    ...
                                    <span class="thumbnail-upload-text">@lang('Upload thumbnail')</span>
                                </label>
                            </div>
                            <div class="thumbnail-preview-wrapper" id="thumbnailPreviewWrapper" style="display: none;">
                                ...
                                <button type="button" class="thumbnail-change-btn" id="changeThumbnailBtn">
                                    <i class="las la-edit"></i> @lang('Change')
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <button type="submit" class="btn btn--base w-100" id="startStreamBtn">
                            <i class="las la-video"></i> @lang('Start Live Stream')
                        </button>
                    </div>
                </form>
                        ...
                        <div class="stream-preview">
                            ...
                            <div class="preview-controls-bar">
                                <div class="controls-left">
                                    ...
                                    <button class="control-btn" id="micToggle" title="@lang('Microphone')">
                                        <i class="las la-microphone"></i>
                                    </button>
                                    <button class="control-btn" id="screenShareBtn" title="@lang('Share screen')">
                                        <i class="las la-desktop"></i>
                                    </button>
                                    <button class="control-btn" id="shareBtn" title="@lang('Share')">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content end-stream-modal-content">
                <div class="modal-body end-stream-modal-body">
                    <h5 class="end-stream-title">@lang('End Stream')</h5>
                    <p class="end-stream-message">@lang('Your stream will stop immediately and you will no longer be live.')</p>
                </div>
                <div class="modal-footer end-stream-modal-footer">
                    <button type="button" class="btn btn-secondary btn-not-yet" data-bs-dismiss="modal">@lang('Not yet')</button>
                    <button type="button" class="btn btn-danger btn-end-confirm" id="confirmEndStreamBtn">@lang('End')</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content stream-finished-content">
                ...
                <div class="stream-finished-body">
                    <div class="finished-video-preview" id="finishedVideoPreview">
                        <div class="video-placeholder">
                            ...
                            <p>@lang('Processing recording...')</p>
                        </div>
                    </div>
                    <div class="finished-stream-info">
                        <h3 id="finishedStreamTitle">-</h3>
                        <p class="broadcaster-name">{{ auth()->user()->channel_name ?? auth()->user()->username }}</p>
                    </div>
                    <div class="stream-analytics">
                        <div class="analytics-grid">
                            <div class="analytics-item">
                                <div class="analytics-label">@lang('Views')</div>
                                <div class="analytics-value" id="analyticsViews">0</div>
                            </div>
                            <div class="analytics-item">
                                <div class="analytics-label">@lang('Avg. view duration')</div>
                                <div class="analytics-value" id="analyticsAvgDuration">0:00</div>
                            </div>
                            <div class="analytics-item">
                                <div class="analytics-label">@lang('Peak concurrents')</div>
                                <div class="analytics-value" id="analyticsPeak">0</div>
                            </div>
                            <div class="analytics-item">
                                <div class="analytics-label">@lang('Total likes')</div>
                                <div class="analytics-value" id="analyticsLikes">0</div>
                            </div>
                            <div class="analytics-item">
                                <div class="analytics-label">@lang('New subscribers')</div>
                                <div class="analytics-value" id="analyticsSubscribers">0</div>
                            </div>
                            <div class="analytics-item">
                                <div class="analytics-label">@lang('Total chats')</div>
                ...
                <div class="stream-finished-footer">
                    <button type="button" class="btn-dismiss" data-bs-dismiss="modal">@lang('Dismiss')</button>
                    <a href="#" id="editInStudioBtn" class="btn-edit-studio">@lang('Edit in studio')</a>
                </div>
            </div>
        </div>
    </div>
...
@push('script')
    <script>
        ...
        thumbnailInput.addEventListener('change', function(e) {
            ...
            if (file) {
                if (file.type.startsWith('image/')) {
                    ...
                    alert('@lang("Please select an image file")');
                    this.value = '';
                }
            }
        });
        ...
        document.getElementById('goLiveForm').addEventListener('submit', async function(e) {
            ...
            startBtn.innerHTML = '<i class="las la-spinner la-spin"></i> @lang("Starting...")';
            ...
            try {
                ...
                const response = await fetch('{{ route("user.live.start") }}', {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    startBtn.innerHTML = '<i class="las la-video"></i> @lang("Start Live Stream")';
                }
            } catch (error) {
        ...
        async function uploadChunk(chunkData, index, isFinal = false) {
            ...
            try {
                ...
                const response = await fetch(`{{ url('user/live/upload-chunk') }}/${streamId}`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    },
        ...
        function startViewerUpdates() {
            setInterval(async () => {
                if (streamId) {
                    try {
                        const response = await fetch(`{{ url('live/stream-info') }}/${streamId}`);
                        const data = await response.json();
        ...
        screenShareBtn.addEventListener('click', async function() {
            try {
                if (!isScreenSharing) {
                    ...
                    this.title = '@lang("Stop sharing screen")';
                ...
                if (error.name === 'NotAllowedError') {
                    alert('@lang("Screen sharing was denied. Please allow screen sharing permission.")');
                } else {
                    alert('@lang("Failed to share screen. Please try again.")');
                }
            }
        });
        ...
        function stopScreenShare() {
            ...
            screenShareBtn.title = '@lang("Share screen")';
        }

        // Share button
        document.getElementById('shareBtn').addEventListener('click', function() {
            if (streamId) {
                const watchUrl = `{{ url('live/watch') }}/${streamId}`;
                if (navigator.share) {
                    navigator.share({
                    ...
                    navigator.clipboard.writeText(watchUrl).then(() => {
                        alert('@lang("Stream URL copied to clipboard!")');
                    }).catch(() => {
                ...
                .then(data => {
                    if (data.success && data.stream) {
                        ...
                        if (stream.recorded_video) {
                            previewDiv.innerHTML = `
                                <video controls style="width: 100%; height: auto; max-height: 400px;">
                                    <source src="{{ url('live/recording') }}/${streamId}" type="video/webm">
                                </video>
                            `;
                        }
                        
                        // Set edit button link
                        document.getElementById('editInStudioBtn').href = `{{ route('user.live.manage') }}`;
                        ...
                        document.getElementById('streamFinishedModal').addEventListener('hidden.bs.modal', function() {
                            window.location.href = '{{ route("user.live.manage") }}';
                        }, { once: true });
                    }
                })
                .catch(error => {
        ...
        document.getElementById('confirmEndStreamBtn').addEventListener('click', async function() {
            ...
            confirmBtn.textContent = '@lang("Ending...")';

            if (streamId) {
                try {
                    // Stop the stream on server IMMEDIATELY (like YouTube)
                    const response = await fetch(`{{ url('user/live/stop') }}/${streamId}`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        }
                    ...
                    if (data.success) {
                        ...
                        alert('@lang("Failed to end stream. Please try again.")');
                        confirmBtn.disabled = false;
                        confirmBtn.textContent = '@lang("End")';
                    }
                } catch (error) {
                    console.error('Error stopping stream:', error);
                    endStreamModal.hide();
                    alert('@lang("Error ending stream. Please try again.")');
                    confirmBtn.disabled = false;
                    confirmBtn.textContent = '@lang("End")';
                }
            } else {
