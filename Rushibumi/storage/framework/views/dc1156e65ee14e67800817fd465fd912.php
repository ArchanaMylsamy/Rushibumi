<div class="upload-details">
    <form class="fetch-youtube-form" method="post">
        <?php echo csrf_field(); ?>
        <label class="form--label"><?php echo app('translator')->get('Fetch Detail From Youtube'); ?></label>
        <div class="input-group ">
            <input class="form-control form--control" name="video_id" type="text" value=""
                placeholder="Youtube Video id">
            <button class="input-group-text btn--base btn btn--sm"><?php echo app('translator')->get('Fetch Data'); ?></button>
        </div>
    </form>
    ...
    <div class="upload-details-wrapper">
                    ...
                    <video class="video-player" controls>
                        <?php if(!$video->is_shorts_video): ?>
                            <?php $__currentLoopData = $video->videoFiles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $file): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <source src="<?php echo e(getVideo($file->file_name, $video)); ?>" type="video/mp4"
                                    size="<?php echo e($file->quality); ?>" />
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php else: ?>
                            <source src="<?php echo e(getVideo($video->video, $video)); ?>" type="video/mp4" />
                        <?php endif; ?>
                    </video>
                </div>
                <div class="uploaded-item__content">

                    <?php if(!$video->storage_id): ?>
                        <canvas id="videoCanvas" style="display:none;"></canvas>
                        <button class="thumbGenerate mt-3">
                            <i class="las la-magic icon"></i> <?php echo app('translator')->get('Generate Thumbnail'); ?>
                        </button>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="upload-details__right">
            <form class="video-upload-form" action="<?php echo e(route('user.' . $action . '.details.submit', $video->id)); ?>"
                method="post" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>
                <div class="form-group">
                    <div class="d-flex justify-content-between">
                        <label class="form--label"><?php echo app('translator')->get('Video Title'); ?></label> <a class="buildSlug"
                ...
                <div class="form-group">
                    ...
                    <input class="form--control checkSlug" name="slug" type="text"
                        value="<?php echo e(old('slug', @$video->slug)); ?>" required>
                </div>

                <?php if(!$video->is_shorts_video): ?>
                    <div class="form-group">
                        <label class="form--label"><?php echo app('translator')->get('Playlist'); ?></label>
                                ...
                                <?php if($video->playlist): ?>
                                    <?php $__currentLoopData = $playlists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $playlist): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e(@$playlist->id); ?>"
                                            <?php if($video->playlist_id == $playlist->id): ?> selected <?php endif; ?>>
                                            <?php echo e(__(@$playlist->title)); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                <?php endif; ?>
                            </select>
                            <button type="button" class="input-group-text btn btn--base adPlayList"
                                data-bs-toggle="modal"><?php echo app('translator')->get('Add New'); ?></button>
                        </div>
                    </div>
                <?php endif; ?>
                <div class="form-group">
                    <label class="form--label"><?php echo app('translator')->get('Video Description'); ?></label>
                    <textarea class="form--control nicEdit" name="description"><?php echo e(old('description', @$video->description)); ?></textarea>
                </div>

                <div class="form-group select2-parent">
                    <label for="" class="form--label"><?php echo app('translator')->get('Only for Playlist'); ?></label>
                    <div class="check-type-wrapper">
                        <label for="playlist-01" class="check-type check-type-success">
                            <input class="check-type-input" type="radio" value="1" name="is_only_playlist"
                                id="playlist-01" <?php if(@$video->is_only_playlist == 1): echo 'checked'; endif; ?>>
                            ...
                            <span class="check-type-label"><?php echo app('translator')->get('Yes'); ?></span>
                        </label>
                        ...
                        <label for="playlist-02" class="check-type check-type-warning">
                            <input class="check-type-input" type="radio" value="0" name="is_only_playlist"
                                id="playlist-02" <?php if(@$video->is_only_playlist == 0): echo 'checked'; endif; ?>>
                            <span class="check-type-icon">
                                <svg class="check-circle" width="13" height="10" viewBox="0 0 13 10"
                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1 5L4.5 8.5L12.5 0.5" stroke="currentColor" stroke-linecap="round"
                            ...
                            <span class="check-type-label"><?php echo app('translator')->get('No'); ?></span>
                        </label>
                    </div>
                </div>
                <div class="upload-thumbnail">
                    <h6 class="upload-thumbnail__title"><?php echo app('translator')->get('Thumbnail'); ?></h6>
                    <p class="upload-thumbnail__desc">
                        <?php echo app('translator')->get('Select or upload a picture that show\'s what\'s in your video. A good thumbnail stands out and draws viewer\'s attention.'); ?>
                        <small class="test"><?php echo app('translator')->get('Thumbnail size will be'); ?>: <?php echo e(getFileSize('thumbnail')); ?> px</small>
                            ...
                            <div class="video-thumb-upload">
                                <div class="video-thumb-upload__avatarPreview">
                                    <div class="video-thumb-upload__thumbnailPreview preview">
                                        <?php if($video->thumb_image): ?>
                                            <img src="<?php echo e(getImage(getFilePath('thumbnail') . '/thumb_' . $video->thumb_image, getFileThumb('thumbnail'))); ?>"
                                                alt="image">
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="video-thumb-upload__avatarEdit">
                                    ...
                                    <label class="video-thumb-upload__box" for="upload-image">
                                        ...
                                        <span class="text"><?php echo app('translator')->get('Upload Thumbnail'); ?></span>
                                    </label><br>
                                </div>
                ...
                <div class="form-group upload-buttons mb-0">
                    ...
                    <?php if(@$video->is_shorts_video): ?>
                        <a class="btn btn--dark"
                            href="<?php echo e(route('user.shorts.upload.form', @$video->id)); ?>"><?php echo app('translator')->get('Previous'); ?> </a>
                    <?php else: ?>
                        <a class="btn btn--dark"
                            href="<?php echo e(route('user.video.upload.form', @$video->id)); ?>"><?php echo app('translator')->get('Previous'); ?> </a>
                    <?php endif; ?>
    ...
    <div class="modal-dialog modal-dialog-centered  modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fs-5" id="thumbModalLabel"><?php echo app('translator')->get('Auto-generated thumbnail'); ?></h5>
                <button class="close modal-close-btn" data-bs-dismiss="modal" type="button" aria-label="Close">
                    <i class="las la-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <p class="mb-3 text"><?php echo app('translator')->get('Select an image from your video to use as a thumbnail'); ?></p>
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn--sm btn--white selectThumb" type="button"><?php echo app('translator')->get('Done'); ?></button>
            </div>
...
<?php echo $__env->make($activeTemplate . 'partials.playlist_modal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        ...
        (function($) {
            ...
            $(document).ready(function() {
                ...
                playlistField.select2({
                    ajax: {
                        url: "<?php echo e(route('user.video.fatch.playlist')); ?>",
                        type: "get",
                        dataType: 'json',
                        delay: 1000,
            ...
            const isShorts = "<?php echo e($video->is_shorts_video); ?>";
            ...
            $(document).on('submit', '.fetch-youtube-form', function(e) {
                e.preventDefault();
                var shorts = "<?php echo e($video->is_shorts_video); ?>"


                $.ajax({
                    type: "post",
                    url: "<?php echo e(route('user.video.fetch.data')); ?>",
                    data: $(this).serialize(),
                    dataType: "json",
            ...
            $(document).ready(function() {
                $('.playlistForm').on('submit', function(e) {
                    ...
                    $.ajax({
                        url: "<?php echo e(route('user.playlist.save')); ?>",
                        method: 'POST',
                        data: $(this).serialize(),
                        success: function(response) {
                            if (response.status == 'success') {
            ...
            $('.checkSlug').on('input', function() {
                ...
                let slug = $(this).val();
                let id = "<?php echo e($video->id); ?>";

                let isShort = 0;
                if (Number("<?php echo e($video->is_shorts_video); ?>")) {
                    isShort = 1;
                }
                ...
                if (slug) {
                    ...
                    closestForm.find('.video-slug-verification').html(`
                        ...
                        `);
                    $.get("<?php echo e(route('user.video.check.slug')); ?>", {
                        slug: slug,
                        id: id,
                        is_short: isShort
                    }, function(response) {
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\laragon\www\Rushibumi\Rushibumi\core\resources\views/templates/basic/partials/video/details.blade.php ENDPATH**/ ?>