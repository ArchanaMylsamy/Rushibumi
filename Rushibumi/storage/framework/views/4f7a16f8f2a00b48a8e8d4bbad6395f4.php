<?php $__env->startSection('content'); ?>
        ...
        <div class="short-video-wrapper">
            <div class="shorts_video_sliders slick-slider">
                ...
                <div class="video-wrapper">
                    <video class="video-player" playsinline preload="metadata" data-video_id="<?php echo e($short->id); ?>" controls>
                        <source src="<?php echo e(route('short.path', encrypt($short->id))); ?>" type="video/mp4" />
                    </video>
                    <div class="action-container">
                        <div class="cmn-button-item">
                            <button class="like-button  button-item reactionBtn" data-video_id="<?php echo e($short->id); ?>" data-reaction="1">
                                <?php if($short->userReactions()->where('user_id', auth()->id())->where('is_like', Status::YES)->exists()): ?>
                            ...
                            <span
                                  class="buton-text likeCount"><?php echo e(formatNumber($short->userReactions()->like()->count())); ?></span>
                        </div>
                        <div class="cmn-button-item">
                            <button class="dislike-button  button-item reactionBtn" data-video_id="<?php echo e($short->id); ?>" data-reaction="0">
                                <?php if($short->userReactions()->where('user_id', auth()->id())->where('is_like', Status::NO)->exists()): ?>
                        ...
                        <div class="cmn-button-item comment">
                            <button class="button-item cmn-btn" data-video_id="<?php echo e($short->id); ?>">
                                <i class="fa-solid fa-message"></i>
                            </button>
                        </div>
                        <div class="cmn-button-item">
                            <button class="button-item shareBtn" data-video="<?php echo e($short); ?>">
                                <i class="fa-solid fa-share"></i>
                            </button>
                        </div>
                        <a class="action-container__thumb" href="<?php echo e(route('preview.channel', $short->user->slug)); ?>">
                            <img src="<?php echo e(getImage(getFilePath('userProfile') . '/' . $short->user->image, isAvatar: true)); ?>" alt="<?php echo app('translator')->get('image'); ?>">
                        </a>
                    </div>
                </div>
                ...
                <?php $__currentLoopData = $relatedVideos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $relatedVideo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="video-wrapper">
                        ...
                        <video class="video-player" playsinline  data-video_id="<?php echo e($relatedVideo->id); ?>" controls>
                            <source src="<?php echo e(route('short.path', encrypt($relatedVideo->id))); ?>" type="video/mp4" />
                        </video>

                        <div class="action-container">
                            <div class="cmn-button-item">
                                <button class="like-button  button-item reactionBtn" data-video_id="<?php echo e($relatedVideo->id); ?>" data-reaction="1">
                                    <?php if($relatedVideo->userReactions()->where('user_id', auth()->id())->where('is_like', Status::YES)->exists()): ?>
                                        <i class="vti-like-fill reactionIcon"></i>
                                    <?php else: ?>
                                        <i class="vti-like reactionIcon"></i>
                                    <?php endif; ?>
                                ...
                                <span
                                      class="buton-text likeCount"><?php echo e(formatNumber($relatedVideo->userReactions()->like()->count())); ?></span>
                            </div>
                            <div class="cmn-button-item">
                                <button class="dislike-button  button-item reactionBtn" data-video_id="<?php echo e($relatedVideo->id); ?>" data-reaction="0">
                                    <?php if($relatedVideo->userReactions()->where('user_id', auth()->id())->where('is_like', Status::NO)->exists()): ?>
                                        <i class="vti-dislike-fill reactionIcon"></i>
                                    <?php else: ?>
                                        <i class="vti-dislike reactionIcon"></i>
                                    <?php endif; ?>
                            ...
                            <div class="cmn-button-item comment">
                                <button class="button-item cmn-btn" data-video_id="<?php echo e($relatedVideo->id); ?>">
                                    <i class="fa-solid fa-message"></i>
                                </button>
                            </div>
                            <div class="cmn-button-item">
                                <button class="button-item shareBtn" data-video="<?php echo e($relatedVideo); ?>">
                                    <i class="fa-solid fa-share"></i>
                                </button>

                            </div>
                            <a class="action-container__thumb" href="<?php echo e(route('preview.channel', $relatedVideo->user->slug)); ?>">
                                <img src="<?php echo e(getImage(getFilePath('userProfile') . '/' . $relatedVideo->user->image, isAvatar: true)); ?>" alt="<?php echo app('translator')->get('image'); ?>">
                            </a>
                        </div>
                    </div>
            ...
            <div class="comment-box">
                <div class="comment-box__header">
                    <h5 class="comment-box__title"><?php echo app('translator')->get('Comments'); ?>: (<span class="buton-text commentCount">0</span>)</h5>
    ...
    <?php echo $__env->make('Template::partials.login_alert_modal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        ...
        (function($) {
            ...
            $(document).ready(function() {
                ...
                function viewsCount() {
                    ...
                    $.ajax({
                        type: "post",
                        url: "<?php echo e(route('short.view', '')); ?>/" + videoId,
                        headers: {
                ...
                $('.comment-form').on('submit', function(e) {
                    ...
                    $.ajax({
                        type: "post",
                        url: "<?php echo e(route('user.comment.submit', '')); ?>/" + videoId,
                        data: $(this).serialize(),
                        dataType: "json",
                        headers: {
                            'X-CSRF-TOKEN': "<?php echo e(csrf_token()); ?>"
                ...
                function loadMoreComments(changeVideo = null, videoId) {
                    const commentsRoute = "<?php echo e(route('user.comment.get', '')); ?>/" + videoId;
                ...
                $('.reactionBtn').on('click', function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?php echo e(route('user.reaction', '')); ?>/" + videoId,
                        dataType: "json",
                        data: {
                            is_like: value,
                        },
                ...
                $(document).ready(function() {
                    $(document).on('click', '.shareBtn', function() {
                        ...
                        const baseUrl = "<?php echo e(route('short.play', '')); ?>";
...
<?php echo $__env->make($activeTemplate . 'layouts.frontend', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Rushibumi\Rushibumi\core\resources\views/templates/basic/shorts_play.blade.php ENDPATH**/ ?>