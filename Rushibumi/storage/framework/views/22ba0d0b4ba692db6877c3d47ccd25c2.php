<?php $__env->startSection('content'); ?>
    ...
    <div class="play-body">
        <div class="play-video">
            <div class="primary ps-0">
                ...
                <div class="primary__videoPlayer video-item__thumb mainVideo" data-live-stream-id="<?php echo e($liveStream->id); ?>">
                    <?php if($isLive): ?>
                        <div id="live-stream-player" class="live-stream-container">
                            <div class="live-stream-placeholder">
                                <div class="stream-status">
                                    ...
                                    <h3><?php echo app('translator')->get('Stream is Live!'); ?></h3>
                                    <p><?php echo app('translator')->get('The broadcaster is currently streaming.'); ?></p>
                                </div>
                            </div>
                    ...
                    <?php elseif($isScheduled): ?>
                        <div class="scheduled-stream">
                            <div class="scheduled-content">
                                ...
                                <h3><?php echo app('translator')->get('Stream Scheduled'); ?></h3>
                                <p><?php echo app('translator')->get('This stream is scheduled to start at'); ?></p>
                                <p class="scheduled-time"><?php echo e($liveStream->scheduled_at ? showDateTime($liveStream->scheduled_at, 'F d, Y h:i A') : '-'); ?></p>
                            </div>
                        </div>
                    <?php elseif($hasRecording && $recordingPath): ?>
                        
                        <video class="video-player" data-amount="0" muted playsinline
                            preload="metadata" 
                            data-db-duration="<?php echo e($liveStream->recorded_duration ?? 0); ?>">
                            <source src="<?php echo e(url('live/recording/' . $liveStream->id)); ?>?t=<?php echo e(time()); ?>" type="video/webm">
                            <source src="<?php echo e(url('live/recording/' . $liveStream->id)); ?>?t=<?php echo e(time()); ?>" type="video/mp4">
                            <?php echo app('translator')->get('Your browser does not support the video tag.'); ?>
                        </video>
                    ...
                    <?php else: ?>
                        <div class="ended-stream">
                            <div class="ended-content">
                                ...
                                <h3><?php echo app('translator')->get('Stream Ended'); ?></h3>
                                <p><?php echo app('translator')->get('This live stream has ended.'); ?></p>
                                <?php if($liveStream->ended_at): ?>
                                    <p class="ended-time"><?php echo app('translator')->get('Ended at'); ?> <?php echo e(showDateTime($liveStream->ended_at, 'F d, Y h:i A')); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endif; ?>
                ...
                <div class="primary__video-content">
                    <h4 class="primary__vtitle"><?php echo e(__($liveStream->title)); ?></h4>

                    <div class="primary__videometa">
                        <div class="items">
                            <span class="view">
                                <span class="icon"><i class="fa-regular fa-eye"></i></span>
                                <?php echo e(formatNumber($liveStream->viewers_count)); ?> <?php echo app('translator')->get('views'); ?>
                            </span>
                            <?php if($liveStream->started_at): ?>
                                <span class="date">
                                    <span class="icon"><i class="fa-regular fa-clock"></i></span>
                                    <?php echo e($liveStream->started_at->diffForHumans()); ?>

                                </span>
                            <?php else: ?>
                                <span class="date">
                                    <span class="icon"><i class="fa-regular fa-clock"></i></span>
                                    <?php echo e($liveStream->created_at->diffForHumans()); ?>
                    ...
                    <div class="primary__channel">
                        <div class="author">
                            <a class="author__thumb" href="<?php echo e(route('preview.channel', $liveStream->user->slug)); ?>">
                                <img src="<?php echo e(getImage(getFilePath('userProfile') . '/' . $liveStream->user->image, isAvatar: true)); ?>"
                                    alt="image">
                            </a>
                            <div class="author__content">
                                <a href="<?php echo e(route('preview.channel', $liveStream->user->slug)); ?>" class="channel-name">
                                    <?php echo e($liveStream->user->channel_name ? $liveStream->user->channel_name : $liveStream->user->fullname); ?>

                                </a>
                                <span class="author__subscriber">
                                    <span class="subscriberCount"><?php echo e(formatNumber($liveStream->user->subscribers()->count())); ?></span>
                                    <?php echo app('translator')->get('Subscriber'); ?>
                                </span>
                            </div>
                        </div>

                        <?php if(auth()->check() && auth()->id() != $liveStream->user_id): ?>
                            <?php
                    ...
                    <div class="primary__desc">
                        <div class="primary__desc-text">
                            <?php if($liveStream->description): ?>
                                <?php echo e(__($liveStream->description)); ?>

                            <?php else: ?>
                                <?php echo app('translator')->get('No description available.'); ?>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="primary__comment d-none d-xl-block">
                        <div class="top">
                            <h5 class="comment-number">
                                <span class="commentCount" id="comments-count">0</span> <?php echo app('translator')->get('Comments'); ?>
                            </h5>
                        </div>
                        <?php if(auth()->check()): ?>
                            <div class="comment-form-wrapper">
                                ...
                                <form class="comment-form" id="commentForm">
                                    ...
                                    <div class="form-group position-relative">
                                        <textarea class="form--control commentBox" name="comment" placeholder="<?php echo app('translator')->get('Add a comment'); ?>"></textarea>
    ...
    ?>
    <?php echo $__env->make('Template::partials.play_video_page_modal', [
        'video' => $videoForModal,
...
<?php $__env->startPush('script'); ?>
    <script>
        const streamId = <?php echo e($liveStream->id); ?>;
        let commentsInterval = null;
        let statusCheckInterval = null;
        let wasLive = <?php echo e($isLive ? 'true' : 'false'); ?>;
        
        // Load comments
        function loadComments() {
            fetch(`<?php echo e(url('live/comments')); ?>/${streamId}`)
        ...
        document.getElementById('commentForm')?.addEventListener('submit', function(e) {
            ...
            fetch(`<?php echo e(url('live/comment')); ?>/${streamId}`, {
                method: 'POST',
                headers: {
        ...
        $(document).ready(function() {
            ...
            if (videoPlayer) {
                ...
                function updateDatabaseDuration(correctDuration) {
                    fetch('<?php echo e(url("live/update-duration")); ?>/<?php echo e($liveStream->id); ?>', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                            'Content-Type': 'application/json',
...
<?php echo $__env->make($activeTemplate . 'layouts.frontend', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Rushibumi\Rushibumi\core\resources\views/templates/basic/live/watch.blade.php ENDPATH**/ ?>